...
- Projects hosted on GitHub or GitLab must reach an [OpenSSF Scorecard](https://github.com/ossf/scorecard) aggregate score of at least 5.0 to be accepted.
...
install_scorecard   download the pinned OpenSSF Scorecard binary to ./bin/
scorecard           check submitted software repositories against OpenSSF Scorecard
...
Maintainers can tune the OpenSSF Scorecard gate through `make` variables: `SCORECARD_THRESHOLD` (default `5.0`), `SCORECARD_STRICT` (also fail when a repo cannot be evaluated), and `SCORECARD_REPO` (check a single `host/owner/name` instead of the pull request diff).
